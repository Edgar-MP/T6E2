...
public class ProcesoPregunta extends HttpServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (request.getParameter("enviar") != null) {
      ...
      int numPreguntas = 0;
      if (Integer.parseInt(request.getParameter("numPreguntas")) > Test.getarrPreguntas().length) {
        numPreguntas = Test.getarrPreguntas().length;
      } else {
        numPreguntas = Integer.parseInt(request.getParameter("numPreguntas"));
      }
      ...
      Test test = new Test(numPreguntas);
      ArrayList<Integer> respuestas = new ArrayList<>();

      session.setAttribute("respuestas", respuestas);
      session.setAttribute("nombre", nombre);
      session.setAttribute("numPreguntas", numPreguntas);
    ...
    try (PrintWriter out = response.getWriter()) {
      ...
      if (numeroPregunta == (int) session.getAttribute("numPreguntas") - 1) {
        out.println("<form method='POST' action='" + request.getContextPath() + "/Resultado'>");
      } else {
        out.println("<form method='POST' action='" + request.getContextPath() + "/ProcesoPregunta'>");
      }
  ...
  public String getServletInfo() {
    return "Servlet ProecesoPregunta";
  }
}
